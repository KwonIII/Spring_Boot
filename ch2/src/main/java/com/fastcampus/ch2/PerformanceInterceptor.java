...
@Component
public class PerformanceInterceptor implements HandlerInterceptor {//단일 책임의 원칙 - 하나의 메서드는 하나의 책임만 갖는다
...
//    long startTime;   //iv - 인스턴스 변수. 싱글톤(하나의 객체)이라서 여러 쓰레드가 하나의 객체를 공유.
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
       ...
       request.setAttribute("startTime",startTime); //request객체에 startTime을 저장
        ...
        System.out.println("mehtod.getBean() = "+method.getBean()); //메서드가 포함된 컨트롤러
